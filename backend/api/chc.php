...
try {
    match ($_SERVER['REQUEST_METHOD']) {
        'GET'          => isset($_GET['id'])
                            ? handleGetOne($pdo, $homeId)
                            : (isset($_GET['resident_id']) ? handleList($pdo, $homeId) : handleSummary($pdo, $homeId)),
...
function handleSummary(PDO $pdo, int $homeId): never
{
    $stmt = $pdo->prepare(
        "SELECT
           SUM(status = 'draft' AND outcome = 'positive') AS positive_pending,
           SUM(status = 'draft')                          AS total_drafts
         FROM chc_checklists
    ...
    $row = $stmt->fetch();
    respond([
        'positive_pending' => (int) ($row['positive_pending'] ?? 0),
        'total_drafts'     => (int) ($row['total_drafts']     ?? 0),
    ]);
}
